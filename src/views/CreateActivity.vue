<template>
  <div>
    ...
    <v-card>
      <div>
        <label for="name">1. 活动信息 <span
                style="color: red">&nbsp;&nbsp;必填*,&nbsp;注意：活动名称只能为字母或者数字的组合，例如 wkkp1、whld2</span></label>
  ...
  export default {
    ...
    methods: {
      ...
      setTime() {
        ...
        let duration = (Date.parse(_this.endTime.time) - Date.parse(_this.startTime.time)) / 60000
        if (duration < config.duration) {
          _this.errmsg = `结束时间必须大于开始时间至少${config.duration}分钟, 请重新设置`
          _this.toastDialog = true
          return false
        }
        let testTimeLength = _this.testTime.length
        if (testTimeLength >= 1) {
          let lastTestTimeEnd = _this.testTime[testTimeLength - 1].end // 最后一个公测的结束时间
          console.log('lastend', lastTestTimeEnd)
          let formalDurationTest = (Date.parse(_this.startTime.time) - Date.parse(lastTestTimeEnd)) / 60000
          if (formalDurationTest < config.testSeconds) {
            _this.errmsg = `开始时间和上一个公测的结束时间必须相差至少${config.testSeconds}分钟,请重新设置！`
            _this.toastDialog = true
            return false
          }
        }
        if (timeType) {
          switch (timeType) {
            case 'formal':
              _this.formalTime.start = _this.startTime.time
              _this.formalTime.end = _this.endTime.time
              _this.showFormal = true
              _this.dateTimeDialog = false
              break;
            case 'test':
              _this.testTime.push({
                start: _this.startTime.time,
                end: _this.endTime.time
              })
              _this.showTest = true
              _this.dateTimeDialog = false
              break;
      ...
      submitCreateActivity() {
        if (this.name === '') {
          ...
          formData.align = JSON.stringify(align)
          const options = {
            method: 'POST',
            headers: {
          ...
          axios(options).then(response => {
            ...
            if (!data.errcode) {
              ...
              _this.toastDialog = true
              setTime(function () {
                _this.toastDialog = false
                _this.$router.push('/home/addOpeningMenu')
              }, 800)
            } else {
