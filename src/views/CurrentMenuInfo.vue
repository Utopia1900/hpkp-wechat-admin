        ...
        <v-card style="box-shadow:none">
          ...
          <v-layout row>
            <v-flex xs8 v-if="editAble">
              <textarea  cols="120" rows="25" style="padding: 10px;border: 1px solid #ccc;" v-model="menuObj"></textarea>
            </v-flex>
            <v-flex xs8 v-else>
    ...
    methods: {
      getCurrentMenuInfo: function () {
        ...
        axios(options).then(response => {
          ...
          if (!data.errcode) {
            this.menuInfoHTML = this.syntaxHighlight(data.menuConfig.selfmenu_info)
            this.menuObj = JSON.stringify(data.menuConfig.selfmenu_info, undefined, 2)
          } else {
...
//          alert(error)
      ...
      setSelfMenu () {
          let menuObj = (typeof this.menuObj !='string') ? JSON.stringify(this.menuObj) : this.menuObj
